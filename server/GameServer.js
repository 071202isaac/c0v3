...
GameServer.prototype.splitCells = function(client, isCannonSplit) {
    ...
    for (var i = 0; i < len; i++) {
		...
		if (isCannonSplit) {
			// Z (turbo split) stays exactly as tuned before - untouched.
			split.setMoveEngineData(this.config.turboSplitSpeed, 32, 0.85);
		} else {
			// Regular space-split: smoother, shorter travel than before, and
			// crucially the SAME speed regardless of how big the cell is -
			// only your base movement should slow down with size, not splits.
			var splitSpeed = this.config.playerSpeed * 2.2;
			split.setMoveEngineData(splitSpeed, 36, 0.88);
		}
...
GameServer.prototype.ejectMass = function(client, isSpamMass) {
    for (var i = 0; i < client.cells.length; i++) {
		...
		if (isSpamMass) {
			// Q (virus-battle balls) stays exactly as it was - already right.
			ejected.setMoveEngineData(this.config.virusBattleEjectSpeed, 20);
		} else {
			// W: smoother, more controlled travel - gentler deceleration
			// spread over more ticks instead of an abrupt stop.
			ejected.setMoveEngineData(this.config.ejectSpeed, 26, 0.90);
		}
...
GameServer.prototype.newCellVirused = function(client, parent, angle, mass, speed, virusType, ticks, decay) {
    // Starting position
    var startPos = {
        x: parent.position.x,
        y: parent.position.y
    ...
    var newCell = new Entity.PlayerCell(this.getNextNodeId(), client, startPos, mass);
    newCell.setAngle(angle);
    // ticks/decay are optional - existing callers (mass overflow auto-split)
    // keep the original quick 15-tick burst; virus-explosion callers pass
    // their own smoother values.
    newCell.setMoveEngineData(speed, ticks || 15, decay);
    switch (virusType)
	{
		...
		default: newCell.calcMergeTime(this.config.playerRecombineTime); break;
	}
    newCell.ignoreCollision = true; // Remove collision checks
    newCell.restoreCollisionTicks = this.config.cRestoreTicks; //vanilla agar.io = 10
    // Add to moving cells list
