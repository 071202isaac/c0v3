...
PlayerCell.prototype.onAutoMove = function(gameServer) {
    ...
    // While a split piece is still "in flight" (space or Z), gently steer
    // it towards the current mouse position each tick. This is a subtle,
    // gradual curve - not an instant snap - so the player can bend the
    // trajectory of a split by moving the mouse while it travels.
    if (this.moveEngineTicks > 0 && this.owner && this.owner.mouse) {
        var dx = this.owner.mouse.x - this.position.x;
        var dy = this.owner.mouse.y - this.position.y;
        if (dx !== 0 || dy !== 0) {
            var targetAngle = Math.atan2(dx, dy);
            var diff = targetAngle - this.angle;
            while (diff > Math.PI) diff -= Math.PI * 2;
            while (diff < -Math.PI) diff += Math.PI * 2;
            this.angle += diff * 0.12; // steering strength
        }
    }
};
