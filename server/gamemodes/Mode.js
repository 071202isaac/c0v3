...
Mode.prototype.pressZ = function(gameServer,player) {
    // Called when the Z key is pressed - split into as many cells as
    // possible (up to playerMaxCells, i.e. a full x16 when mass allows),
    // instead of a fixed double-split.
    var maxCells = gameServer.config.playerMaxCells;
    var previousCount = -1;
    var iterations = 0;
    while (player.cells.length < maxCells && player.cells.length !== previousCount && iterations < 6) {
        previousCount = player.cells.length;
        gameServer.splitCells(player, true);
        iterations++;
    }
};
...
Mode.prototype.pressSpace = function (gameServer, player) {
  // Called when the Space bar is pressed
	if (player.spectate) 
		...
		gameServer.splitCells(player, false);
};
